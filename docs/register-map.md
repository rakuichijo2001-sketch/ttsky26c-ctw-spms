...
| `0x03` | ANOMALY_FAULT_THRESHOLD | `0x20` | R/W | Severe deviation threshold |
...
| `0x07` | PG_STABLE_COUNT | `0x03` | R/W | Consecutive 100 us samples required for a PG state change |
| `0x08` | SEQUENCE_DELAY | `0x05` | R/W | 100 us ticks between rail/load restoration stages |
| `0x09` | STARTUP_TIMEOUT | `0x64` | R/W | 100 us ticks allowed in an unqualified WAIT_PG state |
| `0x0A` | WATCHDOG_TIMEOUT | `0x64` | R/W | Watchdog limit in 100 us ticks |
| `0x0B` | RETRY_DELAY | `0x32` | R/W | Fault retry delay in 100 us ticks |
...
| `0x0E` | WARN_PERSIST_COUNT | `0x02` | R/W | Consecutive warning samples required |
...
| `0x11` | FILTERED_SAMPLE | R | Eight-bit unity-gain moving-average result |
...
| `0x1B` | FAULT_DETAIL | R | Bits 2:0: rail number 1..4 for rail/timeout faults |
| `0x1F` | VERSION | R | Fixed RTL interface version `0x01` |
...
Retry exhaustion is represented by `STATUS.FAULT_LOCK`; it does not overwrite
the physical root cause in `LAST_FAULT`.

Simultaneous fault priority is OVERTEMP, OVERCURRENT, POWER_ANOMALY,
STARTUP_TIMEOUT, PG1_LOSS, PG2_LOSS, PG3_LOSS, PG4_LOSS, then
WATCHDOG_TIMEOUT.
...
- `PG_STABLE_COUNT=0` qualifies a synchronized change after one 100 us sample.
- `SEQUENCE_DELAY=0` adds no intentional delay beyond normal FSM transitions.
...
- `RETRY_DELAY=0` permits retry without an intentional timer delay after
  active physical faults clear.
- `MAX_RETRY=0` disables automatic retry; the root-cause fault remains
  latched until a valid `CLEAR_FAULT`.
